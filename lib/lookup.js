...
function errorLookup(code, os = platform()){
  ...
  {
    if (os === "linux")
      return linuxErrCodes[code] ?? [`Error ${code} (0x${cast(code).toString(16).toUpperCase()})`];
    else if (os === "win32")
      return windowsErrCodes[code] ?? 
             windowsErrCodesHRESULT[cast(code)] ?? //cast signed to unsigned
             [`Error ${code} (0x${cast(code).toString(16).toUpperCase()})`];
    else
